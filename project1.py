...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    LOSS = max(0,(1-(np.dot(feature_vector,theta)+theta_0)*label))
    return LOSS
...
def perceptron_single_step_update(
    ...
    if (label*(np.dot(feature_vector, current_theta)+current_theta_0)<= 1e-9):
...
def classifier_accuracy(
    ...
    train_acc = 1 - np.sum(np.abs(train_pred-train_labels)/2)/train_feature_matrix.shape[0]
    val_pred = classify(val_feature_matrix, theta, theta_0)
    val_acc = 1 - np.sum(np.abs(val_pred-val_labels)/2)/val_feature_matrix.shape[0]
...
def bag_of_words(texts, remove_stopword=False):
    ...
    Returns:
        ...
        integer `index`.
    """
    stopword = np.genfromtxt('stopwords.txt', delimiter=",", dtype= str)
